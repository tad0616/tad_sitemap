...
<{if $all_content|default:false}>
    ...
    <{foreach from=$all_content item=map}>
        ...
        <tr>
            <th id="wcag_item_<{$i|default:''}>"><{$i|default:''}>.<{$map.name}></th>
            <td headers="wcag_item_<{$i|default:''}>">
            <{if $map.item|default:false}>
                <{foreach from=$map.item item=sub}>
                <{if $sub.name|default:false}>
                    <{assign var="j" value=$j+1}>
                    <div>
                    <span class="badge badge-info bg-info"><{$i|default:''}>-<{$j|default:''}></span>
...
<{else}>
    <{if $smarty.session.tad_sitemap_adm|default:false}>
        <div class="jumbotron bg-light p-5 rounded-lg m-3 text-center">
            <a href="<{$xoops_url}>/modules/tad_sitemap/admin/main.php" class="btn btn-info"><i class="fa fa-square-plus" aria-hidden="true"></i>  <{$smarty.const._TAD_ADD}></a>
        </div>
    <{/if}>
<{/if}>